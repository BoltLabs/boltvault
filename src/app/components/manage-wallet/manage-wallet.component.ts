...
export class ManageWalletComponent implements OnInit {
  ...
  async exportAddressBook() {
    ...
    const exportUrl = `http://peer.boltnode.org:9950/import-address-book#${base64Data}`;
  ...
  exportAddressBookToFile() {
    if (this.walletService.walletIsLocked()) return this.notifications.sendWarning(`Wallet must be unlocked`);
    const fileName = `BoltVault-AddressBook.json`;
  ...
  exportToFile() {
    ...
    const fileName = `BoltVault-Wallet.json`;
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (!importData.length || !importData[0].account) {
          return this.notifications.sendError(`Bad import data, make sure you selected a BoltVault Address Book export`)
        }
